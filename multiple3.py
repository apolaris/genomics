...
class sequences:
    ...
    def addextlist(self, t):
        if not t[1]:
            if not t[2]:
                self.extlist.append(t[0][0])
            else:
                self.addextlist(t[2])
        else:
            if not t[2]:
    ...
    def sizeoftree(self, t):
        if(not t[1] and not t[2]):
            ...
            return 1
        if(not t[1] and t[2]):
            size = self.sizeoftree(t[2])
            t[4] = size
            return size
        if(not t[2] and t[1]):
            size = self.sizeoftree(t[1])
            t[4] = size
            return size
        size = self.sizeoftree(t[1]) + self.sizeoftree(t[2])
        t[4] = size
        return size
    ...
    def dfs(self, node):
        ...
        i = node[1][0][0]
        j = node[2][0][0]
        lenext = len(self.ext[i][j])
        if(lenext != 3 and lenext != 5):
            node[3], node[6] = self.multipairwise2(node[1], node[2])
        else:
            node[3] = self.clustalmultipairwise2(node[1], node[2])
    ...
    def refine(self, times):
        ...
        for i in range(0):
            ...
            aligned, realIndex = self.clustalmulti(node1, node2)
    ...
    def clustalmultipairwise2(self, node1, node2):
        a = -self.a
        b = -self.b
        len1 = len(node1[3][0])
        len2 = len(node2[3][0])
        ...
        pena = a
        penb = b
        # if you want to use CLUSTAL with weight, comment thw two lines above and uncomment the two lines below
        #pena = weight_sum * a
        #penb = weight_sum * b
        ...
        for i in range(1, len2 + 1):
            ...
            for j in range(begin, end):
                ...
                for k1 in range(len(node1[3])):
                    for k2 in range(len(node2[3])):
                        score_d += self.compare(node1[3][k1][j - 1], node2[3][k2][i - 1])
                        # if you want to use CLUSTAL with weight, comment the line above and uncomment the line below
                        #score_d += self.compare(node1[3][k1][j - 1], node2[3][k2][i - 1]) * node1[4][k1] * node2[4][k2]
                score_d /= num_pair
                if M[i-1][j-1] >= D[i-1][j-1]:
                    if I[i-1][j-1] > M[i-1][j-1]:
